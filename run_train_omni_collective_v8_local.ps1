...
$outLog = Join-Path $logDir "omni_collective_v8_train_${timestamp}.out.log"
$errLog = Join-Path $logDir "omni_collective_v8_train_${timestamp}.err.log"
$pidPath = Join-Path $logDir "omni_collective_v8_train.pid"
$workerPidPath = Join-Path $logDir "omni_collective_v8_train.worker.pid"
...
$pythonArgs = @(
    "-u"
    "source\train_omni_collective_v8.py"
) + $args
