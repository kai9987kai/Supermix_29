[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$env:HF_HUB_OFFLINE = "1"
$env:TRANSFORMERS_OFFLINE = "1"

python source\qwen_supermix_pipeline.py `
  --data datasets\conversation_data.quality_anchor_v2.jsonl datasets\conversation_data.coding_knowledge_2026_02_19.jsonl datasets\conversation_data.world_events_2026_02_19.jsonl datasets\conversation_data.supermix_plus_v27_500k.jsonl `
  --base_model "C:\Users\kai99\.cache\huggingface\hub\models--Qwen--Qwen2.5-0.5B-Instruct\snapshots\7ae557604adf67be50417f59c2c2f167def9a775" `
  --output_dir artifacts\qwen_supermix_enhanced_v18_fullplus `
  --init_adapter_dir artifacts\qwen_supermix_enhanced_v8_repair\adapter `
  --max_records 2400 `
  --max_source_fraction 0.70 `
  --max_synthetic_fraction 0.05 `
  --max_prompt_signature_count 3 `
  --prompt_signature_cap_exempt_sources "conversation_data.quality_anchor_v2.jsonl" `
  --eval_size 120 `
  --max_length 256 `
  --batch_size 1 `
  --grad_accum_steps 4 `
  --epochs 1 `
  --max_steps 16 `
  --lr 3.0e-5 `
  --weight_decay 0.01 `
  --lora_r 24 `
  --lora_alpha 48 `
  --lora_dropout 0.03 `
  --use_rslora `
  --use_dora `
  --lora_init true `
  --sft_weight_mode quality `
  --sft_min_weight 0.65 `
  --sft_max_weight 1.45 `
  --sft_synthetic_prompt_weight 0.70 `
  --sft_teacher_source_weight 0.92 `
  --sft_quality_anchor_boost 1.08 `
  --sft_coding_boost 1.06 `
  --sft_events_boost 1.06 `
  --sft_min_quality_score 0.95 `
  --sft_quality_filter_exempt_sources "conversation_data.quality_anchor_v2.jsonl,conversation_data.world_events_2026_02_19.jsonl" `
  --sft_auto_balance_sources `
  --sft_source_balance_strength 0.45 `
  --sft_source_balance_max_scale 1.35 `
  --preference_objective repo `
  --preference_steps 8 `
  --preference_pairs 160 `
  --preference_candidate_count 5 `
  --preference_reject_similarity_min 0.15 `
  --preference_beta 2.2 `
  --preference_margin 0.30 `
  --preference_sft_weight 0.22 `
  --preference_length_weight 0.06 `
  --preference_short_reject_boost 0.65 `
  --preference_long_reject_boost 0.20 `
  --preference_min_chosen_quality 0.9 `
  --preference_min_chosen_words 6 `
  --preference_min_quality_gap 0.05 `
  --preference_max_pairs_per_user 1 `
  --preference_max_pairs_per_source 64 `
  --preference_lr 3.0e-5 `
  --preference_max_new_tokens 56 `
  --preference_prompt_max_tokens 224 `
  --supermix_distill_ratio 0.15 `
  --supermix_distill_max 160 `
  --supermix_distill_min_quality 0.90 `
  --seed 42 `
  --device cpu `
  --skip_benchmark
