...
OUTPUT_DIR="${OUTPUT_DIR:-${PERSIST_ROOT}/artifacts/champion_v32_omnifuse_20260326}"
...
PROMPT_DATA_6="${PROMPT_DATA_6:-${PERSIST_ROOT}/artifacts/qwen_supermix_enhanced_v28_cloud_plus_runpod_budget/prepared_train_pairs.jsonl}"
PROMPT_DATA_7="${PROMPT_DATA_7:-${PERSIST_ROOT}/artifacts/qwen_supermix_enhanced_v29_delta_official_refresh_20260326/prepared_train_pairs.jsonl}"
PROMPT_DATA_8="${PROMPT_DATA_8:-${PERSIST_ROOT}/artifacts/qwen_supermix_enhanced_v30_anchor_refresh_20260326/prepared_train_pairs.jsonl}"
...
DISTILL_DATA="${DISTILL_DATA:-${OUTPUT_DIR}/v32_distill_train.jsonl}"
STAGE2_TRAIN_DATA="${STAGE2_TRAIN_DATA:-${OUTPUT_DIR}/v32_stage2_train_mix.jsonl}"
STAGE2_EVAL_DATA="${STAGE2_EVAL_DATA:-${OUTPUT_DIR}/v32_stage2_eval_mix.jsonl}"
DISTILL_SUMMARY="${DISTILL_SUMMARY:-${OUTPUT_DIR}/v32_omnifuse_summary.json}"
DISTILL_AUDIT="${DISTILL_AUDIT:-${OUTPUT_DIR}/v32_omnifuse_audit.jsonl}"

STAGE1_OUT="${STAGE1_OUT:-${OUTPUT_DIR}/champion_model_chat_v32_omnifuse_stage1.pth}"
STAGE1_META="${STAGE1_META:-${OUTPUT_DIR}/chat_model_meta_v32_omnifuse_stage1.json}"
FINAL_OUT="${FINAL_OUT:-${OUTPUT_DIR}/champion_model_chat_v32_omnifuse_final.pth}"
FINAL_META="${FINAL_META:-${OUTPUT_DIR}/chat_model_meta_v32_omnifuse_final.json}"

BUILD_LOG="${BUILD_LOG:-${LOG_DIR}/build_v32_omnifuse.out.log}"
STAGE1_LOG="${STAGE1_LOG:-${LOG_DIR}/train_champion_v32_stage1.out.log}"
STAGE2_LOG="${STAGE2_LOG:-${LOG_DIR}/train_champion_v32_stage2.out.log}"
BENCH_STAGE1_LOG="${BENCH_STAGE1_LOG:-${LOG_DIR}/benchmark_champion_v32_stage1_vs_v31.out.log}"
BENCH_STAGE2_LOG="${BENCH_STAGE2_LOG:-${LOG_DIR}/benchmark_champion_v32_final_vs_stage1.out.log}"
BENCH_FINAL_LOG="${BENCH_FINAL_LOG:-${LOG_DIR}/benchmark_champion_v32_final_vs_v31.out.log}"
SUMMARY_JSON="${SUMMARY_JSON:-${OUTPUT_DIR}/v32_omnifuse_training_summary.json}"
FINAL_ZIP="${FINAL_ZIP:-${PERSIST_ROOT}/champion_v32_omnifuse_final_model_20260326.zip}"
BUNDLE_ZIP="${BUNDLE_ZIP:-${PERSIST_ROOT}/champion_v32_omnifuse_bundle_20260326.zip}"
...
for required in \
  ...
  "${HYBRID_META}"; do
  if [[ ! -e "${required}" ]]; then
    echo "[v32] missing required path: ${required}" >&2
    exit 1
  fi
done
...
BUILD_CMD=(
  python3
  source/build_v32_omnifuse_dataset.py
  --prompt_data
  "${PROMPT_DATA_1}"
  "${PROMPT_DATA_2}"
  "${PROMPT_DATA_3}"
  ...
  --qwen_v29_adapter "${QWEN_V29_ADAPTER}"
  --qwen_v30_adapter "${QWEN_V30_ADAPTER}"
  ...
  --max_new_tokens 96
  --log_every 32
  --min_teacher_score 0.12
)

printf '[v32] build command:\n %q' "${BUILD_CMD[@]}"
printf '\n'
...
STAGE1_CMD=(
  ...
  --output "${STAGE1_OUT}"
  --meta "${STAGE1_META}"
  --model_size neurogenesis_expert
...
printf '[v32] stage1 train command:\n %q' "${STAGE1_CMD[@]}"
printf '\n'
...
printf '[v32] stage1 benchmark command:\n %q' "${BENCH_STAGE1_CMD[@]}"
printf '\n'
...
printf '[v32] stage2 train command:\n %q' "${STAGE2_CMD[@]}"
printf '\n'
...
printf '[v32] stage2 benchmark command:\n %q' "${BENCH_STAGE2_CMD[@]}"
printf '\n'
...
printf '[v32] final benchmark command:\n %q' "${BENCH_FINAL_CMD[@]}"
printf '\n'
...
summary = {
    ...
    "weights_bytes": {
        "start_v31": start_weights_path.stat().st_size,
    ...
    "benchmarks": {
        "v31_vs_stage1_on_eval": parse_benchmark(bench_stage1_path),
        "stage1_vs_final_on_eval": parse_benchmark(bench_stage2_path),
        "v31_vs_final_on_eval": parse_benchmark(bench_final_path),
    },
    "note": (
        "The image-generation variant was distilled indirectly through image-prompt auxiliary rows. "
        "The diffusion renderer itself was not compressed into the Champion text model."
    ),
}
...
python3 - \
  ...
  "${BENCH_STAGE2_LOG}" \
  "${BENCH_FINAL_LOG}" \
  "${REPO_ROOT}/source/build_v32_omnifuse_dataset.py" \
  "${REPO_ROOT}/source/run_train_champion_v32_omnifuse_runpod.sh" <<'PY'
import sys
import zipfile
from pathlib import Path
...
echo "[v32] final model zip: ${FINAL_ZIP}"
echo "[v32] bundle zip: ${BUNDLE_ZIP}"
echo "[v32] summary: ${SUMMARY_JSON}"
